...
class AnalyzerDhadrons_hfcorr(Analyzer):
    ...
    @staticmethod
    def plot_style(histo, residuals):
        histo.GetXaxis().SetTitle("inv_mass D_{0} 1^{st} candidate, GeV/c")
        histo.GetYaxis().SetTitle("inv_mass D_{0} 2^{nd} candidate, GeV/c")
        histo.GetXaxis().SetTitleOffset(2)
        histo.GetYaxis().SetTitleOffset(2)
        histo.SetOption("lego2")
        if residuals:
    ...
    def ub_model(self, ismc, w): # RooFit model for unbinned 2d fit
        ...
        if ismc:
            par = [55000, 17000, 17000, 6000]
            lim_up = [65000, 30000, 30000, 10000]
            lim_low = [40000, 10000, 10000, 1000]
        else:
            par = [200, 15000, 15000, 200000]
            lim_up = [500, 30000, 40000, 400000]
            lim_low = [100, 1000, 1000, 50000]
        ...
        model = w.factory("SUM::model(f0[%f, %f, %f]*sigsig, f1[%f, %f, %f]*sigbkg, f2[%f, %f, %f]*bkgsig, f3[%f, %f, %f]*bkgbkg)" %
                          (par[0], lim_low[0], lim_up[0], par[1], lim_low[1],
                           lim_up[1], par[2], lim_low[2], lim_up[2], par[3],
                           lim_low[3], lim_up[3]))
        print("model created")
        return model
    ...
    def ubfit(self): #unbinned fit processing
        ...
        mass_bin = (self.p_mass_fit_lim[1]-self.p_mass_fit_lim[0])/(binning-1)
...
##################################  MONTE CARLO  ######################################
        ...
        dfreco = dfreco[dfreco["pt_cand1"] <= 24]
        dfreco = dfreco[dfreco["pt_cand2"] <= 24]
        dfreco_mc = dfreco
        ...
        canv = TCanvas("dphi", "", 600, 600)
        dphi_histo = TH1F("dphi plot", "\Delta\phi distibution; \Delta\phi, rad ", 63 , 0, 6.3)
        fill_hist(dphi_histo, dfreco_mc.delta_phi)
        mc_histo = TH2F("DDbar_mc_plot", "2D Invariant mass distribution, MC",
                        binning, self.p_mass_fit_lim[0], self.p_mass_fit_lim[1],
                        binning, self.p_mass_fit_lim[0], self.p_mass_fit_lim[1])
        for i in range(0, len(inv_mass_1)-1):
            mc_histo.Fill(inv_mass_1[i], inv_mass_2[i])
        self.plot_style(mc_histo, False)
        fileout = TFile("fits.root", "RECREATE")
        mc_histo.Write()
        dphi_histo.Draw()
        dphi_histo.Write()
        canv.SaveAs("dphi_mc.png")
...
# plot residuals
        h_residual_abs = TH2F("DDbar residual plot MC abs", "MC residuals (absolute value)",
                              binning, 0, binning, binning, 0, binning)
        h_residual_rel = TH2F("DDbar residual plot MC", "MC residuals (relative value)",
                              binning, 0, binning, binning, 0, binning)
        Nentries = mc_histo.Integral()
        for i in range(1, binning+1):
            xi = float(mc_histo.GetXaxis().GetBinCenter(i))
            x.setVal(xi)
            for j in range(1, binning+1):
                ...
                res = bc - fitv
                if bc > 30:
                    rel_res = int(res*100/bc)
                else: rel_res = 0
                for r in range(0, int(res)):
                   h_residual_abs.Fill(i, j)
                for r in range(0, rel_res):
                   h_residual_rel.Fill(i, j)
            new_label = self.p_mass_fit_lim[0] + (i - 1) * mass_bin
            if i % 9 == 0:
                h_residual_abs.GetXaxis().SetBinLabel(i, str(new_label)[:4])
                h_residual_abs.GetYaxis().SetBinLabel(i, str(new_label)[:4])
                h_residual_rel.GetXaxis().SetBinLabel(i, str(new_label)[:4])
                h_residual_rel.GetYaxis().SetBinLabel(i, str(new_label)[:4])
        cYields_res_mc = TCanvas('cYields_res_mc_rel', 'The Fit Canvas')
        self.plot_style(h_residual_abs, True)
        h_residual_abs.Draw()
        cYields_res_mc.SaveAs("h_DDbar_res_mc_abs.png")
        fileout.cd()
        h_residual_abs.Write()
        cYields_res_mc = TCanvas('cYields_res_mc_rel', 'The Fit Canvas')
        self.plot_style(h_residual_rel, True)
        h_residual_rel.Draw()
        cYields_res_mc.SaveAs("h_DDbar_res_mc_rel.png")
        h_residual_rel.Write()
        fileout.Write()
        fileout.Close()
...
#####################################  DATA  ###########################################
        ...
        dphi_histo = TH1F("dphi plot", "\Delta\phi distribution; \Delta\phi, rad", 63, 0, 6.3)
        fill_hist(dphi_histo, dfreco_data.delta_phi)
        canv = TCanvas("pt", "", 600, 600)
        print("inv mass vect created", len(inv_mass_1))
        data_histo = TH2F("DDbar_data_plot", "2D Invariant mass distribution, data",
                          binning, self.p_mass_fit_lim[0], self.p_mass_fit_lim[1],
                          binning, self.p_mass_fit_lim[0], self.p_mass_fit_lim[1])
        for i in range(0, len(inv_mass_1)-1):
            data_histo.Fill(inv_mass_1[i], inv_mass_2[i])
        self.plot_style(data_histo, False)
        fileout = TFile("fits.root", "UPDATE")
        data_histo.Write()
        dphi_histo.Draw()
        dphi_histo.Write()
        canv.SaveAs("dphi_data.png")
...
# plot residuals
        h_residual_data_abs = TH2F("DDbar residual plot data abs", "Data residuals (absolute value)",
                               binning, 0, binning, binning, 0, binning)
        h_residual_data_rel = TH2F("DDbar residual plot data rel", "residuals (relative value)",
                               binning, 0, binning, binning, 0, binning)
        ...
        for i in range(1, binning+1):
            ...
            for j in range(1, binning+1):
                ...
                if bc > 10:
                    rel_res = int(res*100/bc)
                for r in range(0, int(res)):
                    h_residual_data_abs.Fill(i, j)
                for r in range(0, rel_res):
                    h_residual_data_rel.Fill(i, j)
            new_label = self.p_mass_fit_lim[0] + (i - 1) * mass_bin
            if i % 9 == 0:
                h_residual_data_abs.GetXaxis().SetBinLabel(i, str(new_label)[:4])
                h_residual_data_abs.GetYaxis().SetBinLabel(i, str(new_label)[:4])
                h_residual_data_rel.GetXaxis().SetBinLabel(i, str(new_label)[:4])
                h_residual_data_rel.GetYaxis().SetBinLabel(i, str(new_label)[:4])
        cYields_res_data = TCanvas('cYields_res_data abs', 'The Fit Canvas')
        self.plot_style(h_residual_data_abs, True)
        h_residual_data_abs.Draw()
        cYields_res_data.SaveAs("h_DDbar_res_data_abs.png")
        fileout.cd()
        h_residual_data_abs.Write()
        cYields_res_data = TCanvas('cYields_res_data_rel', 'The Fit Canvas')
        self.plot_style(h_residual_data_rel, True)
        h_residual_data_rel.Draw()
        cYields_res_data.SaveAs("h_DDbar_res_data_rel.png")
        h_residual_data_rel.Write()
        fileout.Close()
        w.Delete()
